...
class Bluetube(object):
    ...
    def set_no_noise(self, no_noise):
    ...
    def _run(self, verbose, show_all):
        ...
        if len(playlists):
            ...
            Bcolors.warn('No playlists in the list.'
                         'Use --add to add a playlist.')
            return False
        return True
    ...
    def _download_and_send_playlist(self, feeds,
        ...
        for ch in playlists:
            ...
            if len(ch['urls']):
                if self._download(ch, download_dir):
                    out_format = ch['playlist']['out_format']
                    title = ch['playlist']['title']
                    if not self._convert_if_needed(out_format,
                                                   title,
                                                   is_converter,
                                                   download_dir):
                        continue
    ...
    def _convert_if_needed(self, out_format,
                           title, is_converter, download_dir):
        ret = True
        if is_converter:
    ...
    def _convert_video(self, download_dir):
        ...
        t = [x for x in files if os.path.splitext(x)[-1] in ['.mp4', '.mkv']]
        for f in t:
            args = ('ffmpeg',) + \
                    ('-i', f) + \
                    options + \
    ...
    def _get_configs(self):
        ...
        ln = len(parser.read(Bluetube.CONFIG_FILES))
        return None if ln == 0 else parser
    ...
    def _check_config_file(self):
        ...
        if not ok:
            with open(Bluetube.CONFIG_TEMPLATE, 'r') as f:
                msg = u'You must create {} or {} ' +\
                    u'with the content below manually:\n{}'
                Bcolors.error(msg.format(Bluetube.CONFIG_FILES[0],
    ...
    def _check_downloader(self):
        if self.executor.does_command_exist(Bluetube.DOWNLOADER):
            ...
            msg = u'ERROR: The tool for downloading from ' +\
                u'youtube "{}" is not found in PATH'
            Bcolors.error(msg.format(Bluetube.DOWNLOADER))
            return False
    ...
    def _get_type(self, out_format):
        if out_format in ['a', 'audio']:
            ...
            Bcolors.error('ERROR: unexpected output type. Should be v '
                          '(or video) or a (audio) separated by SPACE')
        return None

    def _get_feed_url(self, url):
        emsg = u'ERROR: misformatted URL of a youtube list provided.\n' +\
               u'Should be ' +\
               u'https://www.youtube.com/watch?v=XXX&list=XXX ' +\
               u'for a playlist, or ' +\
               u'https://www.youtube.com/feeds/videos.xml?playlist_id=XXX ' +\
               u'for a channel.'
        ...
        if not m:
            m = p2.match(url)
        if m:
            t = 'https://www.youtube.com/feeds/videos.xml?playlist_id={}'
            return t.format(m.group(1))
        else:
            p = re.compile(r'^https://www\.youtube\.com/channel/(.+)$')
            m = p.match(url)
            if m:
                t = 'https://www.youtube.com/feeds/videos.xml?channel_id={}'
                return t.format(m.group(1))
            Bcolors.error(emsg)
            return None
    ...
    def _ask(self, link, summary=None):
        ...
        emsg = u'{}{} to download, {} for reject, {} to get ' +\
               u'a summary (if any), {} to open in a browser.{}'
        # d for download
        d = [u'd', u'D', u'В', u'в', u'Y', u'y', u'Н', u'н', u'yes', u'YES']
        # r for reject
        r = [u'r', u'R', u'к', u'К', u'n', u'N', u'т', u'Т', u'no', u'NO']
        s = [u's', u'S', u'і', u'І']
        open_browser = [u'b', u'B', u'и', u'И']
        question = '{b}d{e}ownload | {b}r{e}eject | open in {b}b{e}rowser'
        question = question.format(b=Bcolors.HEADER,
                                   e=Bcolors.ENDC)
        if summary:
        ...
        while True:
            ...
            if i in d:
                ...
                Bcolors.error(emsg.format(Bcolors.FAIL,
                                          d[0], r[0], s[0], open_browser[0],
                                          Bcolors.ENDC))

    def _get_playlists_with_urls(self, feeds, show_all):
        '''get URLs from the RSS that the user
        will selected for every playlist'''
        all_playlists = feeds.get_all_playlists()
        playlists = []
        ...
        for chs in all_playlists:
            print(chs['author'])
            for ch in chs['playlists']:
                if 'feedparser_data' in ch:
                    print(u'{ind}{tit}'
                          .format(ind=u' ' * Bluetube.INDENTATION,
                                  tit=ch['title']))
                    processed_pl = self._process_playlist(chs['author'],
                                                          ch, show_all)
        ...
        for e in ch['feedparser_data'].entries:
            ...
            if last_update < e_update or show_all:
                if not is_need_update:
                    is_need_update = True
                print(u'{ind}{tit} ({h}:{min:0>2} {d}.{mon:0>2})'
                      .format(**params))
    ...
    def _download(self, playlist, download_dir):
        ...
        args = (Bluetube.DOWNLOADER,) +\
            options + spec_options + tuple(playlist['urls'])
    ...
    def _return_download_dir(self, bluetube_dir):
        if os.path.isdir(bluetube_dir):
            try:
                ...
                m = 'The download directory {} is not empty. Cannot delete it.'
                Bcolors.warn(m.format(bluetube_dir))
                Bcolors.warn('\n  '
                             .join(os.listdir(bluetube_dir))
                             .decode('utf-8'))

    def _get_bt_dir(self):
        p = os.path.join('~', '.bluetube')
        return [Bluetube.CUR_DIR, os.path.expanduser(p), ]
...
def main():
    ...
    me_group.add_argument('--remove', '-r',
                          nargs=2,
                          help='remove a playlist by names of the '
                               'author and the playlist',
    ...
    parser.add_argument('--show_all',
                        action='store_true',
                        help='show all available feed items '
                             'despite last update time')
    parser.add_argument('--yes', '-y',
                        action='store_true',
                        help='answer positive to all questions')
    parser.add_argument('--no_noise',
                        action='store_true',
                        help='don\'t beep when feeds updated')
    ...
    bluetube.set_no_noise(args.no_noise)
    if args.add:
