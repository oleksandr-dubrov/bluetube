...
class FfmpegConvertver(object):
    ...
    def convert(self, entities, configs):
        ...
        if not self._check_video_converter():
            self._event_listener.error('converter not found',
        ...
        for en in entities:
            ...
            if orig == new:
                self._event_listener.warn('conversion is not needed')
                success.append(en)
                continue
            ...
            if not 1 == self._executor.call(args, cwd=self._temp_dir):
                ...
                self._event_listener.error(os.path.basename(orig))
                self._event_listener.inform(f'Command: \n{" ".join(args)}')
                self._event_listener.inform(f'Check {d} after '
